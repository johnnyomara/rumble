...
export const Teams = () => {
  ...
  const { subscribeToMore, data, loading, error } = useQuery(TEAM_QUERY, {
    variables: { id: Number(teamId) },
  });
  ...
  const viewWrestler = (wrestlerId: Number) => {
    navigate(`/rumble/${rumbleId}/${teamNumber}/${wrestlerId}`, {
      state: { id: Number(rumbleId), wrestlerId },
    });
  };
          ...
          team.map((wrestler: any) => {
            return (
              <div>
                ...
                <button onClick={() => viewWrestler(wrestler.wrestlerid)}>
                  Edit
                </button>
              </div>
            );
